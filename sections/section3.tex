...
\draw (-3, 3)--(3, -3);
\draw [<-] (-3, -3)node[below left]{$\bm{n}$}--(3, 3);
\draw (-3.3, 2.7)--(-2.7, 3.3)--(3.3, -2.7)--(2.7, -3.3)--cycle;
\draw [->] (0, 0)--(1, 2.5)[above]node{鏡面反射};
\draw [<-] (4, 1)--+(1, 0);
\draw [<-] (4, 0)--+(1, 0)node[right]{Sun}node[below=12pt,  right]{$P$（輻射圧〔\si{N/m^2}〕）};
\draw [<-] (4, -1)--+(1, 0);
\draw (0.3, 0.75)node[above right]{$\psi$} arc [start angle = 60, end angle = 45, radius = 1cm]; 
\draw (1, 0)node[above right]{$\psi$} arc [start angle = 0, end angle = 45, radius = 1cm]; 
\draw (2.5, -2.6) to[bend right] (1, -2)node[below]{面積$A$};
\end{tikzpicture}
...
= - \frac{\mu_m}{R^3}
\begin{bmatrix}
3 \sin\lambda_m \cos\lambda_m \cos\eta_n \\
3 \sin\lambda_m \cos\lambda_m \cos\eta_n \\
3 \sin^2\lambda_m - 1
\end{bmatrix}
\end{eqaution}
