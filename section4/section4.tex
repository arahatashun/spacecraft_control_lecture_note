...
\end{align}
\input{fig_Omega_simple_vibration}		%置コ消
$\Omega_{max}$が$\Omega_{limit}$に収まるようにする.



\input{fig_Omega_fin_graph}	%置いたらコメント消して
\begin{description}
...
\input{fig_desaturation_zerocross}		%置コ消
...
%\begin{enumerate}
%\item 推定系の世界(仮想)\\
%$\dot{\bm{\omega}} = f(\bm{\omega})\leftarrow \mbox{外乱は含まない}$
%\end{enumerate}
