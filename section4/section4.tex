...
\begin{align}
  ...
  (I_{SP}\omega_{P}+I_{SR}\omega_{R})
  (I_{SP}\dot{\omega_{P}}+
 I_{SR}\dot{\omega_{R}})=0 \\
\end{align}
$\frac{1}{I_T}$倍すると
\begin{align}
...
\end{align}
ここで
$\lambda_P\equiv\omega_n-\omega_P$,
\qquad
$\lambda_R\equiv\omega_n-\omega_R$
と置くと,
\begin{align}
\dot{T}&=
...
\begin{empheq}[left=\empheqlbrace]{align}
 \dot{T_P}=
 -I_{SP}\lambda_P \dot{\omega_{P}}\label{eq:4_7} \\
  \dot{T_R}= 
  -I_{SR}\lambda_R\dot{\omega_{R}}\label{eq:4_8}
\end{empheq}
...
\end{align}
\input{fig_Omega_simple_vibration}
$\Omega_{max}$が$\pm\Omega_{limit}$に収まるようにする.

\input{fig_desaturation_zerocross}
\begin{description}
...
\end{description}
\end{enumerate}
%上の方にpitchのitemがあるけどいまやるとばぐりそうなので暫定的に
...
\end{align}
\footnotetext{$\omega_0 \ll \omega_n$を仮定}
\begin{enumerate}
\item [$+$の方]
\begin{align}
...
\qquad\text{PD制御}
\end{equation}
\footnotetext{バイアスモメンタム衛星のy軸周りと同じ議論}
\end{description}
...
_{柔軟項}
\end{align}
\input{fig_block_board_2}

\input{fig_root_focus_2}
...
%\newpage
%カルマンフィルター\\
%台車の位置$x$を推定するシステムを考える。
%\begin{enumerate}
%\item a = $\ddot{x}$ $\rightarrow$ 2回積分すればx\\
%			aにはノイズ(誤差)が入る\\
 %           $\rightarrow$誤差が発散する
%\item カメラで時々台車の位置を測定する
%\end{enumerate}
%1,2のどちらに重みを置くべきか?

%\input{fig_vehcle_camera}

%\input{fig_probability_distribution}

%\input{fig_probability_update}

%\underline{状態方程式の線形化}---線形化してカルマンフィルターを適用する\qquad EKF
%(Extended Kalman Filter)
%\begin{enumerate}
%\item 推定系の世界(仮想)\\
%$\dot{\bm{\omega}} = f(\bm{\omega})\leftarrow \text{外乱は含まない}$
%\end{enumerate}
